...
// Flask 서버 주소
const char* serverUrl = "http://your-flask-server-ip:5000/api/data";
...
void IRAM_ATTR onTimer() {
  ...
  if (ret) {
    ...
    if (!ecgData.leadoffDetected) {
      ...
      // 데이터 서버로 전송
      sendDataToServer(ecgFilterout, globalHeartRate);
    } else {
      ecgFilterout = 0;
    }
  }
...
void sendDataToServer(int16_t ecg, uint8_t heartRate) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    char payload[100];
    sprintf(payload, "{\"ecg\": %d, \"heartRate\": %d}", ecg, heartRate);
